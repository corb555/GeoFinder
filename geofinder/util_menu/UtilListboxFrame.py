...
from geodata import CachedDictionary
...
class ListboxFrame:
    ...
    def __init__(self, frame, title, dir_name, cache_filename):
        ...
        self.title_label = TKHelper.CLabel(self.frame, text=self.title, width=80, style='Info.TLabel')
        self.status = TKHelper.CLabel(self.frame, text="Highlight items above to remove and click Remove.", width=80, style='Info.TLabel')
    ...
    def configure_widgets(self, frm):
        GridPosition.set_grid_position(self.title_label, "title_label", grd=self.grd)
        GridPosition.set_grid_position(self.status, "status", grd=self.grd)
        GridPosition.set_grid_position(self.tree, "listbox", grd=self.grd)
        GridPosition.set_grid_position(self.scrollbar, "scrollbar", grd=self.grd)
        GridPosition.set_grid_position(self.remove_button, "remove_button", grd=self.grd)
