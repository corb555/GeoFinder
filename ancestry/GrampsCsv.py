...
from geodata import GeoUtil, Loc
...
class GrampsCsv:
    ...
    def get_dict_id(place):
        if place.place_type == Loc.PlaceType.COUNTRY:
            ...
            msg = f'Dictionary ID - Unknown place type for {place.formatted_name}. Type={place.place_type}'
            raise Exception(msg)
        return dict_idx
    ...
    def move_up_level(self, enclosure_place, idx) -> bool:
        ...
        self.geodata.find_first_match(enclosure_place.formatted_name, enclosure_place)
    ...
    def create_enclosed_by(self, place: Loc.Loc):
        ...
        success = self.move_up_level(enclosure_place=enclosure_place, idx=self.get_dict_id(enclosure_place))
        if success:
